...
export default function Purchases() {
  ...
  // Quick-add new supplier without leaving the purchase form
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Open the print-ready invoice view for the new purchase (same as Sales)
      const supplier = suppliers.find(s => s.id.toString() === formData.contact_id.toString())
      const purchaseData = {
        ...res.data,
        contact: supplier,
    ...
    <div className="p-6 space-y-6 animate-fade-in max-w-[1600px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-2">
              <div className="flex items-center justify-between pl-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t.supplier}</label>
                <button
                  type="button"
                ...
                >
                  ...
                  {showNewContact ? (t.cancel || 'Cancel') : (t.new_supplier || 'New supplier')}
                </button>
              </div>
              {!showNewContact ? (
                <select
                ...
                <div className="space-y-2 p-3 border border-sky-100 dark:border-sky-900/40 bg-sky-50/50 dark:bg-sky-900/10 rounded-xl">
                  ...
                  <button type="button" onClick={handleCreateContact} disabled={savingContact} className="w-full btn-primary !py-2 text-sm disabled:opacity-50">
                    {savingContact ? (t.loading || 'Saving...') : (t.save || 'Save supplier')}
                  </button>
                </div>
              )}
            </div>
      ...
      {selectedPurchase && (
        <Modal
          ...
          size="xl"
        >
          <div id="purchase-print-area" className="p-6 sm:p-8 max-w-[850px] mx-auto bg-white text-slate-900 font-sans">
            {/* Header Section */}
            ...
            <div className="flex justify-end mb-16">
              <div className="w-full max-w-md bg-slate-50 rounded-2xl p-6 flex justify-between items-center gap-4">
                ...
                <span className="text-2xl font-black text-slate-900 tracking-tight whitespace-nowrap tabular-nums">{formatCurrency(selectedPurchase.total_amount)}</span>
              </div>
            </div>
